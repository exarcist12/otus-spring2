...
public class BookServiceImpl implements BookService {
    ...
    public BookDto insert(BookCreateDto createDto) {
        ...
        List<Genre> genres = genreRepository.findAllByIdIn(createDto.getGenreIds().stream().toList());
    ...
    public BookDto update(long id, BookUpdateDto updateDto) {
        ...
        List<Genre> genres = genreRepository.findAllByIdIn(updateDto.getGenreIds().stream().toList());
        if (genres.size() != updateDto.getGenreIds().size()) {
            throw new EntityNotFoundException("Some genres not found");
        }
